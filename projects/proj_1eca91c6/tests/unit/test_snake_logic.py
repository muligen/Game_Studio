...
class TestEdgeCases:
    ...
    async def test_rapid_direction_changes(self):
        ...
        # Rapid direction changes (only last one should apply)
        await logic.process_action(session_id, {"type": "direction_change", "direction": "UP"})
        await logic.process_action(session_id, {"type": "direction_change", "direction": "DOWN"})  # Invalid, ignored
        await logic.process_action(session_id, {"type": "direction_change", "direction": "LEFT"})

        game_data = logic._sessions[session_id]
        # Should be LEFT (last valid direction)
        assert game_data.next_direction == Direction.LEFT
